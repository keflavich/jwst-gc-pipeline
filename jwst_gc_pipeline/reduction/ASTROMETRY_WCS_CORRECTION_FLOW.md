...
| 8322 | all | **Gaia** | `TABLE_CONSENSUS` | F200W | omega Cen (NGC 5139), far outside the VVV footprint, so the reference is the field's own `gaia_refcat.fits` (115,009 sources, median NN 1.80" -- denser than the 3" the dense-NN-median rule cares about, so the tie must come from the sanctioned estimators). Unregistered until 2026-09-13: every frame stayed on the raw `assign_wcs` frame (the reduce logged `RA, DE offset:  0.0 0.0`) and `offsets_channel` returned `none`, so m2 could not write what it measured -- the #589 failure. F200W is the band NIRCam and NIRISS share and the only one with full 32-frame NIRCam coverage. 12587, the field's other proposal, is still all planning placeholders and gets no entry |
...
| 3958 | 001, 001-002, 002 | VIRAC2 | `TABLE_CONSENSUS` | F770W | sickle MIRI (obs 001+002, jointly registered as 001-002). Supplies the FRAME, the F770W anchor and — since PR #832 (2026-09-10) — the write channel: `offsets_channel(..., instrument='miri')` now answers from the entry, because `PipelineMIRI.fix_alignment` resolves through `unified_alignment.resolve_shift`. The NIRCam entry above covers only obs 007. Consensus rather than locked because the authored VIRAC2locked table is NIRCam-only. obs 003 belongs to brick |
...
| 1266 | 004, 008, 009, 010 | VIRAC2 | `TABLE_CONSENSUS` | F560W | gc1266, MIRI imaging carried alongside an IFU program (Garcia Marin); four pointings spanning 2.87' about (266.43624, -29.01889). Supplies the FRAME, the anchor and -- since PR #832 (2026-09-10) -- the write channel: `PipelineMIRI.fix_alignment` resolves through `unified_alignment.resolve_shift`. gc1266 is what forced that open; obs 004 and 009 measured 4.28" and 5.70" off VIRAC2 (both confirmed against sparse Gaia to 18 and 10 mas) while 008 and 010 tie at 8-13 mas. Consensus rather than locked because nothing was measured for 1266 before that |
| 3571 | 001, 003, 004, 005, 016 | VIRAC2 | `TABLE_CONSENSUS` | F560W | gc3571, MIRI imaging of the CMZ (Yusef-Zadeh); five adjoining tiles (1/2/3/4/6, Tile-5 never delivered) spanning 3.22' about (266.41973, -29.00509). Same reasoning as 1266 above: frame, anchor and write channel. F560W anchors both because MIRI has one pixel scale, so the choice is which band gives the best-measured consensus, and F560W is closest to VIRAC2's Ks and detects the most stars -- the longer MIRI bands are dust-emission dominated in the CMZ |
| 9257 | 001 | VIRAC2 | `TABLE_CONSENSUS` | F212N | gc9257, the nuclear star cluster (Do); one observation carrying NIRCam F212N+F480M and a NIRISS F140M pointing ~6' NW, epoch 2025.691. Registered 2026-09-17 with the field itself, BEFORE the first reduce, for the reason the 10678 row gives. F212N anchors: the shorter NIRCam band, what `consensus_catalog.reference_filter` ranks first for this list, and the closer of the two to VIRAC2's Ks. Consensus rather than locked because nothing has been measured for 9257 yet. The NIRISS half is NOT table-correctable -- PR #832 opened MIRI's write channel, NIRISS still has none -- so F140M reduces at its dispatched frame |
