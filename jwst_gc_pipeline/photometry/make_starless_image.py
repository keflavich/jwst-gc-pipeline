...
def measure_all_mask_radii(data, wcs, catalog, filt_lower):
    ...
    # ----- Vectorised setup -----
    # Pull every per-source quantity needed by the loop below into plain
    # numpy arrays in one shot, instead of indexing astropy Columns
    # cell-by-cell inside the hot loop (the previous version did
    # ``catalog[col][i]`` inside ``for i in range(len(catalog))``, which
    # ran a Python-level Column.__getitem__ per cell per filter and
    # dominated the loop time on iter3-class merged catalogs).
    ...
    def _col_as_float(name, fill):
        """Return ``catalog[name]`` as a plain float ndarray with masked
        entries replaced by ``fill``.  Returns full-fill array if the
        column is absent."""
    ...
    if has_flux:
        flux_arr = _col_as_float(flux_col, 0.0)
        err_arr  = _col_as_float(fluxerr_col, global_sigma) if has_err \
                   else np.full(n, global_sigma, dtype=float)
    else:
        flux_arr = np.zeros(n, dtype=float)
        err_arr  = np.full(n, global_sigma, dtype=float)
    with np.errstate(invalid='ignore', divide='ignore'):
        snr_arr = np.where(err_arr > 0, flux_arr / err_arr, 0.0)

    # Max SNR across all available filters (vectorised reduction over a
    # stack of per-filter snr arrays).
    all_filt_pairs = [(f'flux_{f}', f'fluxerr_{f}')
                      for f in ['f187n', 'f210m', 'f335m', 'f470n', 'f480m']
                      if f'flux_{f}' in catalog.colnames
                      and f'fluxerr_{f}' in catalog.colnames]
    if all_filt_pairs:
        snr_stack = [snr_arr]
        for fc, ec in all_filt_pairs:
            f_arr = _col_as_float(fc, np.nan)
            e_arr = _col_as_float(ec, np.nan)
            with np.errstate(invalid='ignore', divide='ignore'):
                snr_stack.append(np.where(e_arr > 0, f_arr / e_arr, 0.0))
        max_snr_arr = np.nanmax(np.vstack(snr_stack), axis=0)
    else:
        max_snr_arr = snr_arr

    is_sat_arr = (np.array(catalog['is_saturated'], dtype=bool)
                  if 'is_saturated' in catalog.colnames
                  else np.zeros(n, dtype=bool))

    cx_arr = np.round(px_arr).astype(int)
    cy_arr = np.round(py_arr).astype(int)
    in_bounds = (cx_arr >= 0) & (cx_arr < nx) & (cy_arr >= 0) & (cy_arr < ny)

    # ----- Per-source loop (real per-source work only) -----
    # ``measure_mask_radius_cutout`` cuts a stamp around each source and
    # walks radial bins; this is genuine per-source numpy work that can't
    # be batched without restructuring the cutout API.  Everything else
    # is now precomputed above.
    for i in range(n):
        if not in_bounds[i]:
            continue
        if is_singular[i]:
            ...
            max_r = _max_r_for_source(flux_arr[i],
                                      max_snr_arr[i],
                                      is_sat_arr[i])
        if max_r == 0:
            continue
        radii[i] = measure_mask_radius_cutout(data, cy_arr[i], cx_arr[i], max_r)
        if (i + 1) % 1000 == 0:
            n_done = int((radii[:i + 1] >= MIN_MASK_RADIUS).sum())
            print(f'    {i+1}/{n} sources measured; '
