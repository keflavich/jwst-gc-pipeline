...
def _load_ramp_cube(crf_path):
    """Load the full ramp cube (all groups) + GROUPDQ for a per-frame crf/cal
    file from its sibling Detector1 ``_ramp.fits``.

    Returns ``(sci, groupdq)`` where ``sci`` is ``(ngroups, ny, nx)`` DN (first
    integration) and ``groupdq`` the matching per-group DQ (or None if absent),
    or ``None`` if no ramp is found / not NIRCam.  Used by the ramp-SLOPE
    saturated-rim recovery (``ramp_recover_saturated``), the crowding-immune
    successor to the group-0 zeroframe recovery."""
    import re
    ramp = re.sub(r'(_nrc[ab](?:long|[1-4]))_.*\.fits$', r'\1_ramp.fits',
                  str(crf_path))
    if ramp == str(crf_path) or not os.path.exists(ramp):
    ...
    if sci is None:
        return None
    sci = sci[0] if sci.ndim == 4 else sci   # -> (ngroups, ny, nx)
    if sci.ndim != 3:
        return None
    try:
...
class FinalizeModuleCoverageError(RuntimeError):
    """A finalize was asked for fewer modules than the field has on disk."""


#: Which module families a NIRCam ``--modules`` entry stands for.  MIRI
#: (``mirimage``) and NIRISS (``nis``) are single-detector instruments with no
#: module split, so they have no family here and the coverage check does not
#: apply to them.
...
_PERFRAME_OBS_TOKEN_ONLY_RE = re.compile(r'\A(?:o\d{3}(?:-\d{3})?|j\d{4,5})\Z')


def perframe_module_family(basename, filt):
    """The module FAMILY a per-frame catalog basename belongs to.

    ``f212n_nrcb1_o037_visit001_vgroup...`` -> ``('nrcb', 'o037')``;
    ``f770w_mirimage_visit001_vgroup...``   -> ``('mirimage', '')``.

    Per-frame products are named by the real DETECTOR, never by the requested
    module (``file_module = file_detector`` in the phase loop, so four SW
    detectors do not collapse onto one filename), so the family is what the
    detector token starts with.  Returns ``(None, '')`` for a basename this
    cannot read rather than guessing.
    """
    head = os.path.basename(basename).split('_visit', 1)[0]
    parts = head.split('_')
    if len(parts) < 2 or parts[0].lower() != str(filt).lower():
        return None, ''
    parts = parts[1:]
    obs = ''
    if _PERFRAME_OBS_TOKEN_ONLY_RE.match(parts[-1]):
        obs = parts[-1]
        parts = parts[:-1]
    token = '_'.join(parts).lower()
    if not token:
        return None, ''
    for fam in NIRCAM_MODULE_FAMILIES:
        if token.startswith(fam):
            return fam, obs
    return token, obs


def requested_nircam_families(modules):
    """The NIRCam module families a ``--modules`` list covers.

    ``None`` means "everything": ``merged`` expands, in ``get_filenames``, to
    every detector token of the filter (both modules for NIRCam, ``mirimage``
    for MIRI), so a run that asks for it is not truncated whatever is on disk.
    An empty set means the run asked for no NIRCam module at all (a MIRI or
    NIRISS run, including one carrying the ``--modules nrcb`` sbatch default),
...
def perframe_families_on_disk(cut_bp, filt, merge_label, obs_token=''):
    """NIRCam module families with per-frame ``merge_label`` catalogs on disk.

    Returns ``{family: example basename}``, restricted to
    :data:`NIRCAM_MODULE_FAMILIES` -- a single-detector instrument cannot be
    module-truncated, and a NIRCam-shaped ``--modules`` on a MIRI filter (the
    sbatch default is ``nrcb``) must not read ``mirimage`` as a missing module.

    Scoped to this run's observation BY NAME.  ``obs_token`` is what the
    per-frame writer emits (``crowdsource_catalogs_long.obs_token``): ``_o037``
    for 10678's 139 tiles sharing one tree, empty for everything else.  A
    catalog spelling a different token belongs to another tile, and reading it
    would refuse tile 037 for tile 042's modules.  No header is read: the
    question is which modules were fitted, and the name answers it.
    """
    want_obs = str(obs_token or '').lstrip('_')
    ...
    for fn in fns:
        ...
        if not accept.search(bn):
            continue
        fam, obs = perframe_module_family(bn, filt)
        if fam not in NIRCAM_MODULE_FAMILIES or obs != want_obs:
            continue
        found.setdefault(fam, bn)
    return found
...
def assert_finalize_covers_disk_modules(cut_bp, modules, filternames,
    ...
    for filt in filternames:
        have = perframe_families_on_disk(cut_bp, filt, merge_label,
                                         obs_token=obs_token)
        for fam, example in sorted(have.items()):
            if fam not in want:
                missing.setdefault(fam, []).append((filt, example))
    if not missing:
        return
    detail = '\n'.join(
    ...
    if getattr(options, 'allow_partial_modules', False):
        if os.environ.get('ALLOW_PARTIAL_MODULES') == '1':
            print(msg + "\n  (--allow-partial-modules + ALLOW_PARTIAL_MODULES=1 "
                        "-- continuing)", flush=True)
            return
        raise FinalizeModuleCoverageError(
            msg + "\n  --allow-partial-modules ALONE is not enough: it also "
                  "requires ALLOW_PARTIAL_MODULES=1 in the environment.")
    raise FinalizeModuleCoverageError(msg)
...
def _drop_foreign_obs_duplicates(fns, obs_token, filt, merge_label, module,
    ...
    ``obs_token`` here is the token the per-frame WRITER
    (``crowdsource_catalogs_long.obs_token``) emits, not the one the consensus
    catalog is named with.  Only a token that was written can be matched; see
    the call site for what filtering on the consensus token instead costs.

    Whether a pre-token basename is ambiguous depends on the FILTER, so the
    registry decides (``filter_observation_count``):

    - **more than one observation images this filter** -- an untokened basename
    ...
    if shared:
        ...
        # exposure_key; comparing raw basenames would keep both and reach
        # DuplicateExposureError by a different route.
        def _identity(base):
            m = _OBS_TOKEN_RE.search(base)
            if m:
                base = base.replace('_' + m.group(1), '', 1)
        ...
        for ident, group in untokened.items():
            ...
            if ident in tokened:
                ...
                drop.extend(group[1:])
        for ident, group in tokened.items():
            group = sorted(group)
            # Two DIFFERENT tokens on a filter only one observation images (or
            # the same exposure chunked and unchunked) still collide on
            # exposure_key.  Keep one.
            drop.extend(group[1:])
    if drop:
...
def _run_astrometry_stage_checkpoint(merge_label, module, filt, cut_bp, basepath,
    ...
    #
    # Two DIFFERENT tokens, and they must not be confused:
    #
    # `_obs_token` names the consensus catalog this run WRITES.  Those files are
    # new, so `consensus_obs_token` gives every observation its own name --
    # including proposals whose per-frame writer emits nothing, which is why it
    # falls back to `_o{obsid}` unconditionally.
    #
    ...
    # 72 -> 0, sickle F187N 192 -> 0, silent at m2 and fatal at m3+.  For
    # gc2211 and ngc6334 -- the fields this filter exists for -- the two helpers
    # return the same string, so the drop is unchanged there (32 and 120 kept).
    _obs_token = consensus_obs_token(getattr(options, 'proposal_id', None),
                                     getattr(options, 'field', None))
    _perframe_token = _perframe_obs_token(getattr(options, 'proposal_id', None),
                                          getattr(options, 'field', None))
    base = f"{cut_bp}/{filt.upper()}/{filt.lower()}_*visit*_vgroup*_exp*"
    fns = sorted(set(
        glob.glob(f"{base}_{merge_label}_daophot_basic.fits")
...
def _stamp_catalog_provenance(path, stage, options, parent_paths=None):
    """Best-effort provenance sidecar for a merged/combined catalog product.

    Records the pipeline tag, stage, code + params fingerprints, the output facet
    hashes, and -- when ``parent_paths`` is given -- the UPSTREAM output facets
    read from the parent products' sidecars (``{parent_stage: path | [paths]}``),
    which is what drives the rerun-skip cascade.  Written next to ``path``
    (``<path>.prov.json``) with the compact keys mirrored into the FITS.

    FAIL-SOFT: provenance must never break cataloging, so every failure mode
    ...
    try:
        ...
        from jwst_gc_pipeline.versioning import upstream as _vup
    except ImportError:
        return
    params = None
    ...
    if parent_paths:
        try:
            upstream = _vup.upstream_from_sidecars(parent_paths) or None
        except (OSError, ValueError, KeyError):
            upstream = None
...
def _marker_is_current(marker_path, frame_path, seed_inputs=()):
    ...
    marker untouched, so an existence test cannot tell a finished frame from a
    frame whose only recorded fit predates it.

    Measured on the live tree (#570).  ``gc2211_o046`` on 2026-08-26 carried, in
    one marker directory:

        480 markers written that day, matching the frames on disk
        ~720 markers from 2026-08-11, while every `_crf` dated 2026-08-25
             (regenerated by reduce jobs 40201196 / 40246068)

    An existence-only resume reading the second group skips frames whose only
    recorded fit is of a frame deleted two weeks later, and reports success.
    That is worse than the refit it saves.  brick carried 977 of 2400 in the same
    state.
...
def select_resumable_frames(frame_args, marker_dir, filt, phase, merge,
    ...
    * ``resumed_nooverlap`` -- ``(filename, reason)`` for legitimate misses.
    ...
    frame on the affected trees.  Note that accepting it also WIDENS what this
...
def run_manual_pipeline(options, modules, filternames, nvisits, proposal_id,
    ...
    # observation, are errors; a band the observation never took is dropped.
    ...
    #
    # It runs HERE, ahead of `multifilter` / `phases` / `is_miri`, because a
    # skipped filter has to LEAVE `filternames`: left in the list it walks into
    # the phase loop with zero frames and raises `no {filt}/{module} frames
    # produced output in phase {phase}` -- the raise the not-observed verdict
    # exists to avoid -- and the derived decisions below (m7, the MIRI schedule)
    # must be taken on the filters that actually run.
    _preflight_frames = {}
    ...
    if _drop_filters:
        filternames = [_f for _f in filternames if _f not in _drop_filters]
    ...
    for phase in phases:
        ...
        for module in modules:
            for filt in filternames:
                ...
                # provenance: the seed parent of this phase is the PREVIOUS
                # phase's merged catalog (same module/filt).  m12 has no
                # cataloging parent -- its parent is the imaging frames, whose
                # upstream-frame pooling is a follow-up.
                _prev_phase = (phases[phases.index(phase) - 1]
                               if phase in phases and phases.index(phase) > 0
                               else None)
                _prev_merged = (
                    _merged_path('m2' if _prev_phase == 'm12' else _prev_phase,
                                 module, filt, resbgsub)
                    if _prev_phase else None)
    ...
    if _do_crossband and not getattr(options, 'cutout_region', ''):
        ...
        for module in modules:
            ...
            # which mutates the file); stamp as stage 'm7'.  Its seed parent is
            # the POOL of every filter's m6 merged catalog (the cross-band seed
