...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-gns",
        action="store_true",
        help="Skip GALACTICNUCLEUS catalog fetch and bootstrap (use VVV-only). "
             "Required for fields outside GNS coverage (e.g. Sgr C).",
    )
    parser.add_argument(
        "--vvv-max-sep-arcsec",
        type=float,
...
def main() -> None:
    ...
    if args.skip_gns:
        gns = None
        print("--skip-gns set: skipping GALACTICNUCLEUS fetch and bootstrap.")
    else:
