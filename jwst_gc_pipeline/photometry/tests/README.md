...
| --- | --- | --- |
| crowdsource_catalogs_long.py | Star B masked `(0,0)` sentinel after vstack | `test_seed_skycoord_resolution.py` (pre-existing) |
| crowdsource_catalogs_long.py | SeededFinder must log silent finite-drops | `test_seed_skycoord_resolution.py` (pre-existing) |
| crowdsource_catalogs_long.py | `overlap_slices` ndarray-shape + `e_max==0` ambiguous-truth crash | `test_crowdsource_long_regressions.py::TestOverlapSlicesPatch` |
| crowdsource_catalogs_long.py | `_chunkXXofYY` suffix stripping (`is_iter3` must still fire) | `…::TestStripChunk` |
| crowdsource_catalogs_long.py | per-filter `skycoord_{filter}` snap must beat plain ra/dec (sickle F480M src 55) | `…::TestResolveSeedPreferredColumn` |
| crowdsource_catalogs_long.py | Star B saturation-proximity radius 5.0→1.0 px | `…::TestFilterNearSaturation` |
| crowdsource_catalogs_long.py | closed-form forced-photometry flux solve | `…::TestForcedPsfPhotometry` |
...
| crowdsource_catalogs_long.py | `--max-group-size` 0 rejected as ambiguous; `unlimited`→None; +int passthrough | `test_crowdsource_long_regressions.py::TestResolveMaxGroupSize` |
| crowdsource_catalogs_long.py | `normalize_vgroup_id` token/int extraction, idempotent on prefixed | `…::TestNormalizeVgroupId` |
...
- **iter3 `xy_bounds` / finder settings** (crowdsource_catalogs_long.py):
  seed-dominated iter3 must not wander far from seeded positions.
- **`save_photutils_results` filename token** (crowdsource_catalogs_long.py):
...
- **Memory two-phase refactor** (merge_catalogs.py:356-380) and **SeededFinder
  vectorised-skycoord perf** (crowdsource_catalogs_long.py): these
