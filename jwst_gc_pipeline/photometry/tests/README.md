...
Note: importing `catalog_long` / `merge_catalogs` pulls in
...
| --- | --- | --- |
| catalog_long.py:1140 | Star B masked `(0,0)` sentinel after vstack | `test_seed_skycoord_resolution.py` (pre-existing) |
| catalog_long.py:1870 | SeededFinder must log silent finite-drops | `test_seed_skycoord_resolution.py` (pre-existing) |
| catalog_long.py:54-91 | `overlap_slices` ndarray-shape + `e_max==0` ambiguous-truth crash | `test_crowdsource_long_regressions.py::TestOverlapSlicesPatch` |
| catalog_long.py:693-707 | `_chunkXXofYY` suffix stripping (`is_iter3` must still fire) | `…::TestStripChunk` |
| catalog_long.py:1180-1188 | per-filter `skycoord_{filter}` snap must beat plain ra/dec (sickle F480M src 55) | `…::TestResolveSeedPreferredColumn` |
| catalog_long.py:1456-1507, ~4657 | Star B saturation-proximity radius 5.0→1.0 px | `…::TestFilterNearSaturation` |
| catalog_long.py:199-296 | closed-form forced-photometry flux solve | `…::TestForcedPsfPhotometry` |
...
| catalog_long.py:449-477 | `--max-group-size` 0 rejected as ambiguous; `unlimited`→None; +int passthrough | `test_crowdsource_long_regressions.py::TestResolveMaxGroupSize` |
| catalog_long.py:888-903 | `normalize_vgroup_id` token/int extraction, idempotent on prefixed | `…::TestNormalizeVgroupId` |
...
| catalog_long.py:`do_photometry_step` (blocks A-U) | END-TO-END characterization (Phase-6 safety net): basic / iterative / seeded paths each recover 3 injected stars <1px on a synthetic frame (4 external seams stubbed) | `test_do_photometry_step_integration.py` |
| catalog_long.py:`_output_suffix_tokens` | filename-suffix tokens (block B), field-order locked | `test_crowdsource_long_regressions.py::TestOutputSuffixTokens` |
| catalog_long.py:`_first_pass_daofinder` | iter1 DAO threshold = nsigma·min(median err, mad_std data) (block J else) | `…::TestFirstPassDaofinder` |
...
- **V12 iter2 source injection, Modes A & B** (catalog_long.py:3908-3962):
...
- **V12 satstar snap-to-iter2** (catalog_long.py:4028-4041):
...
- **Post-fit dedup thresholds** (catalog_long.py:4604-4615,
...
- **iter3 `xy_bounds` / finder settings** (catalog_long.py:3641-3655):
  seed-dominated iter3 must not wander far from seeded positions.
- **`save_photutils_results` filename token** (catalog_long.py:2239-2247):
...
- **Memory two-phase refactor** (merge_catalogs.py:356-380) and **SeededFinder
  vectorised-skycoord perf** (catalog_long.py:1857-1864): these
