...
#: where it was and takes the bulk out from under it: in AGGREGATE the per-cell
#: error goes from hypot(bulk, structure) to structure.
#:
#: Per cell that is not automatic, and the ratio is what makes the trade
#: favourable rather than the aggregate argument (PR #924 review).  A cell whose
#: true offset is zero goes from 0 to |bulk| -- and that cell is the undisplaced
#: half of a seam, which is precisely the case the veto exists for.  Bounding
#: the worst cell at half the bulk bounds that harm at 0.5*bulk while the
#: typical cell gains ~bulk, so the shift is worth making; unbounded, it is not.
#:
#: What the bound excludes: a SEAM, where one region is displaced and the
#: measured "bulk" is a median over displaced and undisplaced parts.  Take a
#: two-level seam, fraction f of the field displaced by D.  The adopted bulk is
#: a MEDIAN (`local_residual_map`, and the map's own is np.median at
#: astrometry_offsets.py:1261), not a mean, so it does not slide with f -- it
#: sits at 0 for f < 0.5 and at D for f > 0.5, while the worst cell residual is
#: D either way.  The ratio is therefore D/0 -> unbounded below f = 0.5 and
#: pins at exactly 1.00 above it.  It never approaches 0.5 at any f, so no pure
#: two-level seam can pass this bound -- a stronger statement than a
#: displaced-fraction threshold, and the reason not to quote one.
#:
#: Real examples land where that predicts: brick-1182 F200W's ~90 mas strip sits
#: on a sub-mas bulk, and the synthetic half-mosaic in
#: test_reference_tie_unswept_sparse_alias.py reads a 2500 mas worst cell
#: against a sub-mas bulk.  Both are ratios in the hundreds.
#:
...
#:
#: 0.5 admits those with margin, and sits a factor of two below the 1.00 a
#: two-level seam pins at -- see above for why that pin is exact rather than a
#: function of the displaced fraction.
#:
...
def measure_reference_tie(consensus_coords, ref_coords_all, ref_coords_sparse,
    ...
    #
    # Confined to the RESIDUAL arm as well, which the ratio alone does not do.
    # `clean` is `not flagged and not uncovered`, and those two arms catch
    # opposite failures: a seam INSIDE the match radius shows up as a large
    # cell residual, and a region displaced BEYOND it keeps its sources, loses
    # its pairs, and contributes NO cell -- it lands in `n_uncovered` and never
    # reaches `worst_off_mas`, which is a max over MEASURED cells.  So the
    # coverage arm's signature is a SMALL numerator, which is exactly what this
    # exemption looks for: brick-1182 v001's ~20" half-mosaic would sail
    # through on a ratio near zero.  The ratio speaks only about residuals, so
    # it may only overrule residuals (PR #924 review).
    ...
    # Recorded, not folded into the ratio: a refusal should still report how the
    # residuals compared to the bulk even when it was coverage that blocked.
    region_uncovered = int((per_tile_same_star or {}).get("n_uncovered") or 0)
