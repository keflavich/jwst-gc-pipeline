...
recurring failure mode behind the 2221/1182 astrometry errors. The brick-1182 v001
~20" error came from an offsets-table CURATION collapse (v001 overwritten with
v002's value) — the same class of silent false-agreement failure, so NN-median
against a dense catalog is banned outright.
...
### ⚠ Histogram-stacking carries a dense-reference bias

The offset-histogram peak is unbiased only when the wrong-pair background is
uniform. Against a **DENSE** reference (full VIRAC2), two catalogs tracing the SAME
clustered stellar field make a correlated, clumpy wrong-pair background that
**pulls the peak by several mas** — a second way density fools the estimator.
Measured on brick (2026-07-16): histogram vs dense VIRAC2 read **~9–10 mas with a
±6.5 mas RA term whose SIGN FLIPS between filters** — a sign flip per filter marks
it as an artifact — while the **same-star** tie was ~0 in RA / ~−5 mas in Dec.
Against SPARSE Gaia the histogram was clean (matched same-star).
(Memory: `histogram-vs-samestar-offset-bias`.)
...
small global tie already exists (so the nearest pair is the RIGHT star).
`measure_reference_tie` does this automatically and
reports `bulk_source` (`"same-star"` / `"histogram"`); cycle-N table corrections must
come from the same-star bulk.
...
The sparse-Gaia cross-check retains ONLY a GROSS gate
(`REFERENCE_CROSSCHECK_GROSS_MAS`, ~100 mas) to catch a spurious/window-limited
VIRAC peak (the brick-1182 v001 ~700 mas tell).
(Memory: `gc-gaia-frame-not-catalog`.)

### A bulk offset ≈ 0 can hide a shifted half-mosaic
...
broken. Sign-off requires that map.

### A LOW contrast can mean "offset ≫ window"
...
- `measure_offset` **sweeps the window** (3→10→30→60") by default and takes the
...
- **A swept peak near the window EDGE is a geometry artifact** (issue #158). Two
  ...
  expect to be small — it re-measures at an independent window and accepts only a
  peak that reproduces there. One accepted alias CASCADES: the displaced exposure
...
SIP only approximates the JWST distortion, and both directions of that fit carry
error:

- **the fit residual**: 5–8 mas on every frame written before 2026-07-29,
  *position-dependent and different per detector and per filter*, so a bulk tie
  leaves it in place. Cause: `gwcs.WCS.to_fits()` defaults to `max_pix_error=0.25`
  **px** (STScI uses 0.01), and the reduction re-stamped headers with a bare
  `header.update(ww.to_fits()[0])`. In pixels that is up to ~165 mpix. SIP's own
  forward→inverse round trip closes to 0.000 mpix, so the whole error is in the fit.
- **off-footprint**: the iterative SIP inverse either **raises `NoConvergence`**
  (the W51 m8 abort) or, with `quiet=True`, returns **finite garbage silently** —
  which then lands in a catalog.

The GWCS is exact in both directions (inverse to <1 mpix) and returns `NaN`
off-footprint on every call path, at comparable speed (forward ~1.3× slower,
inverse ~1.1× faster).
...
- `i2d` mosaics are exempt — `resample` writes a rectified plain `RA---TAN` grid,
  so `WCS(i2d_header)` is exact.
- `astropy.wcs.WCS(header)` on a frame must always pass `relax=True`: a header whose
  CTYPE lost the `-SIP` suffix still carries `A_*`/`B_*`, and `relax=True` is what
  reads them; the default drops them silently (~0.1" at the detector corners).
...
exists only for deliberate, justified overrides.

**⛔ Inter-frame overlap check is BLOCKING and needs its own gate.**
...
- `registration_failsafes.py` matches the mosaic vs its **own merged catalog** —
  ...
  **cancels** (both wrong the same way → agree → PASS). Its unswept ±2.5″ search
  also covers offsets up to 2.5″ only. A green `registration_failsafes` therefore
  still needs the overlap gate below.
- A **field-pooled** offset (one number per visit/mosaic) averages a spatially
  ...
  (≥12×12) and gate on offset **magnitude** as well as contrast.
...
- New photometry code goes in new modules; the `crowdsource_catalogs_long.py`
  monolith stays as it is.
