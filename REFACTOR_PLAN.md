...
## Phase 3 — STScI / library redundancy (NEW — audit complete)
Biggest WCS wins. **NOTE: origin/main @ 318b902 "propagate correction into gwcs" may have
already changed some of this — re-verify each before editing.**

Tier 1 (WCS, high value, verify numbers):
- `reduction/align_to_catalogs.py:21-65` `sync_gwcs_to_fits_wcs` — hand-builds GWCS to patch a
  FITS-header-only shift; has a silent no-op hole (60-63). Replace whole fn with
  `jwst.tweakreg.utils.adjust_wcs(model.meta.wcs, dra, ddec)` + `model.save`.
