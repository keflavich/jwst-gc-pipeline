...
| `default_reference_catalog` | The catalog consulted for any observation that has no exact `reference_catalog` key. An exact key still wins, and an observation with neither still raises. Use it only where ONE catalog is right for every observation of the proposal: a `reference_catalog` key is looked up by obsid string and works under a wildcard `obsids` too, so a wildcard field is not a reason to reach for it. Program 10678 (139 tiles over 1.28 x 0.71 deg) carried one here and got one catalog for all of them; it now registers per tile instead -- see `scripts/reduction/build_treasury_refcats.py`. |
