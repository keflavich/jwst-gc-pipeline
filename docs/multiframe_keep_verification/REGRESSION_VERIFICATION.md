...
default). The verification below establishes that scope limit.
...
daofind (red) is worse than per-frame, so the gain lies in survival.
...
- Multi-frame keep (5 new): default-off; admit faint multi-frame star; respect qfit
  ...
  source and keeps the tight (3 mas) one. (This exercises the guard mechanism; real
  extended emission has a *stable* centroid and passes it — see the cutout section.)
...
### Extended-emission cutouts — the multi-frame keep re-admits emission
...
dark-filament +110 sources are emission knots: median prominence 0.9
(95% below 3), sitting on bright background — the same low-prominence emission the
recover prominence gate rejects. pillar_head gained a fake as well.
...
The bg medians go *down* on these fields because the emission is wrongly
subtracted into the model, which lowers the residual.
...
**Scope of the feature:** the multi-frame keep is a *star-field* tool.
...
- **(c) recovery vs OUR nmatch** — the keep works: `our_nmatch≥3` recovers 76–99%.
  The residual misses are dominated by `our_nmatch=0` (24,001 — undetected,
...
- **(d) satstar spikes** — 11,416 of 28,648 (40%) are within 1.5″ of a saturated
  star (unmeasurable spike zones / Hosek artifacts).
...
faint stars). The remaining gap is the *detection* floor — ~24k missed entirely
...
ndet=3. Closing further needs better detection in crowded/spike regions.
